...
def plot_bar(savings, usr):
    ...
    df = pd.DataFrame({"year": year, "savings": savings})
    ...
    # Set the main title
    plt.suptitle(
        "Maras Projected Savings",  # Main title text
...
def plot_simulation_percentiles(usr, sav_sim):
    ...
    ax.yaxis.set_major_formatter("${x:1.2f}")
    ax.plot(
        np.percentile(sav_sim, 25, axis=0), c="r", linewidth=2, label="Percentile=25"
    )
    ax.plot(
        np.percentile(sav_sim, 50, axis=0), c="b", linewidth=2, label="Percentile=50"
    )
    ax.plot(
        np.percentile(sav_sim, 75, axis=0), c="g", linewidth=2, label="Percentile=75"
    )

    # Set the main title
    plt.suptitle(
        "Monte Carlo Simulation of Maras Projected Savings",  # Main title text
        fontsize=18,  # Set the font size
        color="black",  # Set the color
        x=0.51,  # Adjust this to align with the subtitle
        y=1.01,  # Adjust this to align with the subtitle
