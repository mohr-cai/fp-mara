...
def savings_fixed_policy(usr: Type[Profile], contrib=0.1, seed=None) -> List[float]:
    ...
    for year_ in range(usr.target_year):
        ...
        usr.current_balance = GBM(
            usr.current_balance, usr.portfolio_return, usr.portfolio_stdv
        ).sample()
