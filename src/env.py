...
import numpy as np
from src.distributions import GBM
from src.reward import reward_function
...
class PlanEnv(gym.Env):
    ...
    def __init__(self, usr, one_hot):
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=1, shape=(len(one_hot) + 1,), dtype=np.float16
        )
        self.usr = usr
    ...
    def step(self, action):
        ...
        self.current_balance = GBM(
            self.current_balance, self.usr.portfolio_return, self.usr.portfolio_stdv
        ).sample()
        reward = reward_function(self.current_balance, self.usr.goal_amount)
        self.current_balance += this_year_saving
        self.total_steps += 1
        self.current_step += 1
        done = (self.current_step == self.usr.target_year) | (
            self.current_balance >= self.usr.goal_amount
        )
        obs = np.array(
            [self.current_balance / self.SCALE_FACTOR] + self.one_hot[self.current_step]
        )
    ...
    def render(self, mode="human", close=False):
        print(
            f"Step: {self.current_step} current_balance: {self.current_balance} action: {self.action}"
        )
